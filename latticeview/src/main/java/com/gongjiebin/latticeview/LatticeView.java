...
public class LatticeView extends BaseLatticeView {
    ...
    public void loadView() {
        ...
        mRedText = new ArrayList<>();
    }

    public void showMenuRedText(int position, String count) {
        if (mRedText != null && mRedText.size() >= (position + 1)) {
            mRedText.get(position).setVisibility(View.VISIBLE);
            mRedText.get(position).setText(count);
        }
    }

    public void hideMenuRedText(int position) {
        if (mRedText != null && mRedText.size() >= (position + 1)) {
            mRedText.get(position).setVisibility(View.GONE);
            mRedText.get(position).setText("");
        }
    }
        ...
        for (int i = 0; i < countLine; i++) {
            ...
            for (int j = 0; j < lineImage.length; j++) {
                ...
                if (!TextUtils.isEmpty(lineText[j])) {
                    ...
                    RelativeLayout.LayoutParams imageIr;
                    RelativeLayout.LayoutParams ir;
                    if (imageTextBeanParams.imageWidth != 0 && imageTextBeanParams.imageHigh != 0) {
                        ir = new RelativeLayout.LayoutParams(dip2px(getContext(), imageTextBeanParams.imageWidth), dip2px(getContext(), imageTextBeanParams.imageHigh));
                        imageIr = new RelativeLayout.LayoutParams(dip2px(getContext(), imageTextBeanParams.imageWidth + 30), dip2px(getContext(), imageTextBeanParams.imageHigh + 12));
                    } else {
                        ir = new RelativeLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                        imageIr = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                    }
                    if (imageTextBeanParams.imageType != null) {
                        imageView.setScaleType(imageTextBeanParams.imageType);
                    }
                    ir.addRule(RelativeLayout.CENTER_IN_PARENT);
                    imageView.setLayoutParams(ir);
                    RelativeLayout relativeLayout = new RelativeLayout(getContext());
                    relativeLayout.setLayoutParams(imageIr);
                    relativeLayout.addView(imageView);

                    TextView redText = new TextView(getContext());
                    final int wh = LatticeView.dip2px(getContext(), 14);
                    RelativeLayout.LayoutParams textLp = new RelativeLayout.LayoutParams(wh, wh);
                    textLp.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
                    redText.setGravity(Gravity.CENTER);
                    redText.setLayoutParams(textLp);
                    redText.setClipToOutline(true);
                    redText.setOutlineProvider(new ViewOutlineProvider() {
                        @Override
                        public void getOutline(View view, Outline outline) {
                            outline.setRoundRect(0, 0, wh, wh, wh / 2);
                        }
                    });
                    redText.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 8);
                    redText.setBackgroundColor(getResources().getColor(android.R.color.holo_red_dark));
                    redText.setTextColor(getResources().getColor(android.R.color.white));
                    redText.setVisibility(View.GONE);
                    relativeLayout.addView(redText);
                    mRedText.add(redText);
                    imageTextBeanParams.imageLoader.displayImage(mContext, lineImage[j],imageView);
                    lio.addView(relativeLayout, 0);
                    ...
                    if (imageTextBeanParams.textSize != 0)
                        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, imageTextBeanParams.textSize);
    ...
    public void changeTextSize(int position, int textSize, int textSelectSize) {
        if (textViews != null) {
            for (int i = 0; i < textViews.size(); i++) {
                ...
                if (position == i) {
                    // 点击选中的
                    textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, textSelectSize);
                } else {
                    // 其它的恢复原状
